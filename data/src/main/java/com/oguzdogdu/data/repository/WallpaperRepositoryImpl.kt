...
import com.oguzdogdu.data.model.searchdto.toSearchDomain
...
class WallpaperRepositoryImpl @Inject constructor(private val service: WallpaperService) :
    ...
    override suspend fun getImagesByPopulars(page: Int?): List<PopularImage> {
        return service.getImagesByOrders(order = Constants.POPULAR, apiKey = API_KEY, page = page)
            .map {
    ...
    override suspend fun getImagesByLatest(page: Int?): List<LatestImage> {
        return service.getImagesByOrders(order = Constants.LATEST, apiKey = API_KEY, page = page)
            .map {
